...
export class Tournament {
  ...
  #createMatches(players: Array<Player>) {
    const format =
      this.status === "stage-one" ? this.stageOne.format : this.stageTwo.format;
    let matches = [];
    switch (format) {
      ...
      case "stepladder":
        if (format === "single-elimination") {
          matches = Pairings.SingleElimination(
            players.map((p) => p.id),
            this.round,
            this.stageOne.consolation,
            this.status === "stage-one" ? this.sorting !== "none" : true
          );
        } else if (format === "double-elimination") {
          matches = Pairings.DoubleElimination(
            players.map((p) => p.id),
            this.round,
            this.status === "stage-one" ? this.sorting !== "none" : true
          );
        } else if (format === "stepladder") {
          matches = Pairings.Stepladder(
            players.map((p) => p.id),
            this.round,
            this.status === "stage-one" ? this.sorting !== "none" : true
          );
        ...
        matches.forEach((match) => {
          ...
          do {
            ...
            this.matches.some((m) => m.id === id) ||
            newMatches.some((m) => m.id === id)
          );
          ...
          if (newMatch.player1.id !== null && newMatch.player2.id !== null) {
            this.players
              .find((p) => p.id === match.player1.toString())
              .addMatch({
                id: id,
                opponent: match.player2.toString(),
              });
            this.players
              .find((p) => p.id === match.player2.toString())
              .addMatch({
                id: id,
        ...
        newMatches.forEach((match) => {
          const origMatch = matches.find(
            (m) => m.round === match.round && m.match === match.match
          );
          const winPath = origMatch.hasOwnProperty("win")
            ? newMatches.find(
                (m) =>
                  m.round === origMatch.win.round &&
                  m.match === origMatch.win.match
              ).id
            : null;
          const lossPath = origMatch.hasOwnProperty("loss")
            ? newMatches.find(
                (m) =>
                  m.round === origMatch.loss.round &&
                  m.match === origMatch.loss.match
              ).id
            : null;
          match.values = {
            path: {
      ...
      case "double-round-robin":
        ...
        matches.forEach((match) => {
          ...
          if (match.round === this.round) {
            if (newMatch.player1.id === null || newMatch.player2.id === null) {
              ...
              this.players
                .find((p) => p.id === newMatch.player1.id)
                .addMatch({
                  id: id,
                  opponent: newMatch.player2.id,
                });
              this.players
                .find((p) => p.id === newMatch.player2.id)
                .addMatch({
                  id: id,
        ...
        if (format === "double-round-robin") {
          matches = Pairings.RoundRobin(
            players.map((p) => p.id),
            this.matches.reduce((max, curr) => Math.max(max, curr.round), 0) +
              1,
            this.status === "stage-one" ? this.sorting !== "none" : true
          );
          matches.forEach((match) => {
      ...
      case "swiss":
        const playerArray = players.map((player) => ({
          id: player.id,
          score: player.matches.reduce(
            (sum, match) =>
              match.win > match.loss
                ? sum + this.scoring.win
                : match.loss > match.win
                ? sum + this.scoring.loss
                : sum + this.scoring.draw,
            0
          ),
          pairedUpDown: player.matches.some(
        ...
        matches.forEach((match) => {
          ...
          if (newMatch.player2.id !== null) {
            const player1Points = this.players
              .find((p) => p.id === newMatch.player1.id)
              .matches.reduce(
                (sum, curr) =>
                  this.matches.find((m) => m.id === curr.id).active === true
                    ? sum
                    : curr.win > curr.loss
                    ? sum + this.scoring.win
                    : curr.loss > curr.win
            ...
            const player2Points = this.players
              .find((p) => p.id === newMatch.player2.id)
              .matches.reduce(
                (sum, curr) =>
                  this.matches.find((m) => m.id === curr.id).active === true
  ...
  #computeScores(): Array<StandingsValues> {
    ...
    for (let i = 0; i < playerScores.length; i++) {
      ...
      player.player.matches
        .filter((match) =>
          this.matches.find((m) => m.id === match.id && m.active === false)
        )
        .forEach((match) => {
          player.gamePoints +=
            (match.bye ? this.scoring.bye : this.scoring.win) * match.win +
            this.scoring.loss * match.loss +
            this.scoring.draw * match.draw;
      ...
      player.tiebreaks.gameWinPct =
        player.games === 0
          ? 0
          : player.gamePoints / (player.games * this.scoring.win);
      player.tiebreaks.matchWinPct =
        player.matches === 0
          ? 0
          : player.matchPoints / (player.matches * this.scoring.win);
    }
    for (let i = 0; i < playerScores.length; i++) {
      const player = playerScores[i];
      const opponents = playerScores.filter((p) =>
        player.player.matches.some((match) => match.opponent === p.player.id)
      );
      if (opponents.length === 0) {
        continue;
      }
      player.tiebreaks.oppMatchWinPct =
        opponents.reduce((sum, opp) => sum + opp.tiebreaks.matchWinPct, 0) /
        opponents.length;
      player.tiebreaks.oppGameWinPct =
        opponents.reduce((sum, opp) => sum + opp.tiebreaks.gameWinPct, 0) /
        opponents.length;
      ...
      if (oppMatchPoints.length > 2) {
        ...
        player.tiebreaks.medianBuchholz = oppMatchPoints.reduce(
          (sum, curr) => sum + curr,
          0
        );
      }
      ...
      player.tiebreaks.oppCumulative = opponents.reduce(
        (sum, opp) => sum + opp.tiebreaks.cumulative,
        0
      );
    }
    for (let i = 0; i < playerScores.length; i++) {
      ...
      player.tiebreaks.oppOppMatchWinPct =
        opponents.reduce((sum, opp) => sum + opp.tiebreaks.oppMatchWinPct, 0) /
        opponents.length;
    }
    return playerScores;
  }
  ...
  createPlayer(name: string, id: string | undefined = undefined): Player {
    if (
      (this.status === "stage-one" && this.stageOne.format !== "swiss") ||
    ...
    if (
      this.stageOne.maxPlayers > 0 &&
      this.players.length === this.stageOne.maxPlayers
    ) {
  ...
  removePlayer(id: string): void {
    ...
    if (
      (this.status === "stage-one" &&
        ["single-elimination", "double-elimination", "stepladder"].includes(
          this.stageOne.format
        )) ||
      (this.status === "stage-two" &&
        ["single-elimination", "double-elimination", "stepladder"].includes(
          this.stageTwo.format
        ))
    ) {
      const activeMatch = this.matches.find(
        (match) =>
          match.active === true &&
      ...
      if (activeMatch !== undefined) {
        const opponent = this.players.find(
          (p) =>
            p.id ===
            (activeMatch.player1.id === player.id
        ...
        if (activeMatch.path.win !== null) {
          const winMatch = this.matches.find(
            (match) => match.id === activeMatch.path.win
          );
          ...
          if (winMatch.player1.id !== null && winMatch.player2.id !== null) {
            ...
            this.players
              .find((p) => p.id === winMatch.player1.id)
              .addMatch({
                id: winMatch.id,
                opponent: winMatch.player2.id,
              });
            this.players
              .find((p) => p.id === winMatch.player2.id)
              .addMatch({
                id: winMatch.id,
        ...
        if (activeMatch.path.loss !== null) {
          const lossMatch = this.matches.find(
            (match) => match.id === activeMatch.path.loss
          );
          if (lossMatch.player1.id === null && lossMatch.player2.id === null) {
            const prevMatch = this.matches.find(
              (match) =>
                (match.path.win === lossMatch.id ||
                  match.path.loss === lossMatch.id) &&
            ...
            prevMatch.values = {
              path: {
                win:
                  prevMatch.path.win === lossMatch.id
                    ? lossMatch.path.win
                    : prevMatch.path.win,
                loss:
                  prevMatch.path.loss === lossMatch.id
                    ? lossMatch.path.win
                    : prevMatch.path.loss,
              },
            };
            ...
            const waitingPlayer = this.players.find(
              (player) =>
                player.id ===
                (lossMatch.player1.id === null
                  ? lossMatch.player2.id
                  : lossMatch.player1.id)
            );
            const winMatch = this.matches.find(
              (match) => match.id === lossMatch.path.win
            );
            if (winMatch.player1.id === null) {
              winMatch.values = {
                player1: {
      ...
      const waitingMatch = this.matches.find(
        (match) =>
          (match.player1.id === player.id && match.player2.id === null) ||
          (match.player2.id === player.id && match.player1.id === null)
      );
      if (waitingMatch !== undefined && waitingMatch.path.win !== null) {
        const prevMatch = this.matches.find(
          (match) =>
            (match.path.win === waitingMatch.id ||
              match.path.loss === waitingMatch.id) &&
            match.player1.id !== player.id &&
            match.player2.id !== player.id
        );
        prevMatch.values = {
          path: {
            win:
              prevMatch.path.win === waitingMatch.id
                ? waitingMatch.path.win
                : prevMatch.path.win,
            loss:
              prevMatch.path.loss === waitingMatch.id
                ? waitingMatch.path.win
                : prevMatch.path.loss,
          },
        };
        if (waitingMatch.path.loss !== undefined) {
          const prevLossMatch = this.matches.find(
            (match) =>
              (match.path.win === waitingMatch.path.loss ||
                match.path.loss === waitingMatch.path.loss) &&
              match.player1.id !== player.id &&
              match.player2.id !== player.id
          );
          const currLossMatch = this.matches.find(
            (match) => match.id === waitingMatch.path.loss
          );
          prevLossMatch.values = {
            path: {
              win:
                prevLossMatch.path.win === currLossMatch.id
                  ? currLossMatch.path.win
                  : prevLossMatch.path.win,
              loss:
                prevLossMatch.path.loss === currLossMatch.id
                  ? currLossMatch.path.win
                  : prevLossMatch.path.loss,
            },
          };
        }
      }
    } else if (
      ["round-robin", "double-round-robin"].includes(this.stageOne.format)
    ) {
      const byeMatches = this.matches.filter(
        (match) =>
          match.round > this.round &&
          (match.player1.id === player.id || match.player2.id === player.id)
      );
  ...
  start(): void {
    ...
    if (
      (this.stageOne.format === "double-elimination" && players.length < 4) ||
      players.length < 2
    ) {
    ...
    if (this.sorting !== "none") {
      players.sort((a, b) =>
        this.sorting === "ascending" ? a.value - b.value : b.value - a.value
      );
    }
    this.status = "stage-one";
    ...
    if (this.stageOne.format === "swiss" && this.stageOne.rounds === 0) {
      ...
      this.stageOne.rounds = this.matches.reduce(
                ...
                player.matches.reduce(
                  (sum, match) =>
                    match.win > match.loss
                      ...
                      : this.scoring.draw,
                  0
                ) < this.stageTwo.advance.value
            )
            .forEach((player) => (player.active = false));
        } else if (this.stageTwo.advance.method === "rank") {
          const standings = this.standings();
          standings.splice(0, this.stageTwo.advance.value);
          standings.forEach(
            (s) =>
              (this.players.find((p) => p.id === s.player.id).active = false)
          );
        }
        if (
          (this.stageTwo.format === "double-elimination" &&
            this.players.filter((player) => player.active === true).length <
              4) ||
          this.players.filter((player) => player.active === true).length < 2
        ) {
          throw `Insufficient number of players (${
            this.players.filter((player) => player.active === true).length
          }) to create stage two matches`;
        }
        this.#createMatches(
          this.standings()
            .map((s) => s.player)
      ...
      if (
        ...
        matches.forEach((match) => {
          if (match.player1.id === null || match.player2.id === null) {
              ...
              player1: {
                win:
                  match.player2.id === null
                    ? Math.ceil(this.scoring.bestOf / 2)
                    : 0,
              },
              player2: {
                win:
            ...
            this.players
              .find((p) => p.id === match.player1.id)
              .addMatch({
                id: match.id,
                opponent: match.player2.id,
              });
            this.players
              .find((p) => p.id === match.player2.id)
              .addMatch({
                id: match.id,
  ...
  enterResult(
    ...
    if (
      player1Wins > Math.round(this.scoring.bestOf / 2) ||
      player2Wins > Math.round(this.scoring.bestOf / 2)
    ) {
      throw `Players can not win more than ${Math.round(
        this.scoring.bestOf / 2
      )} games in a match`;
    }
    match.values = {
      active: false,
    ...
    if (match.path.win !== null) {
      const winID =
        player1Wins > player2Wins ? match.player1.id : match.player2.id;
    ...
    const lossID =
      player1Wins > player2Wins ? match.player2.id : match.player1.id;
    if (match.path.loss !== null) {
      ...
      if (lossMatch.player1.id !== null && lossMatch.player2.id !== null) {
        ...
        this.players
          .find((p) => p.id === lossMatch.player1.id)
          .addMatch({
            id: lossMatch.id,
            opponent: lossMatch.player2.id,
          });
        this.players
          .find((p) => p.id === lossMatch.player2.id)
          .addMatch({
            id: lossMatch.id,
  ...
  clearResult(id: string): void {
    ...
    if (match.path.win !== null) {
      ...
      if (winMatch.active === true) {
        this.players
          .find((player) => player.id === winMatch.player1.id)
          .removeMatch(winMatch.id);
        this.players
          .find((player) => player.id === winMatch.player2.id)
          .removeMatch(winMatch.id);
      }
      winMatch.values = {
        active: false,
        player1: {
          id:
            winMatch.player1.id === player1.id ||
            winMatch.player1.id === player2.id
              ? null
              : winMatch.player1.id,
        },
        player2: {
          id:
            winMatch.player2.id === player1.id ||
            winMatch.player2.id === player2.id
              ? null
              : winMatch.player2.id,
        },
      };
    ...
    if (match.path.loss !== null) {
      ...
      if (lossMatch.active === true) {
        this.players
          .find((player) => player.id === lossMatch.player1.id)
          .removeMatch(lossMatch.id);
        this.players
          .find((player) => player.id === lossMatch.player2.id)
          .removeMatch(lossMatch.id);
      }
      lossMatch.values = {
        active: false,
        player1: {
          id:
            lossMatch.player1.id === player1.id ||
            lossMatch.player1.id === player2.id
              ? null
              : lossMatch.player1.id,
        },
        player2: {
          id:
            lossMatch.player2.id === player1.id ||
            lossMatch.player2.id === player2.id
              ? null
              : lossMatch.player2.id,
        },
      };
  ...
  assignBye(id: string, round: number): void {
    ...
    if (
      player.matches.some(
        (match) => this.matches.find((m) => m.id === match.id).round === round
      )
    ) {
  ...
  assignLoss(id: string, round: number): void {
    ...
    if (
      ...
      const currentMatch = this.matches
      ...
      this.matches.splice(
        this.matches.findIndex((match) => match.id === currentMatch.id),
        1
      );
      ...
      const opponent = this.players.find(
        (p) =>
          p.id ===
          (currentMatch.player1.id === player.id
      ...
      for (let i = 0; i < this.scoring.tiebreaks.length; i++) {
        switch (this.scoring.tiebreaks[i]) {
          ...
          case "opponent opponent match win percentage":
            if (
              a.tiebreaks.oppOppMatchWinPct !== b.tiebreaks.oppOppMatchWinPct
            ) {
              return (
                b.tiebreaks.oppOppMatchWinPct - a.tiebreaks.oppOppMatchWinPct
              );
            } else continue;
        }
      }
